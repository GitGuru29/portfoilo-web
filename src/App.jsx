...
const MapNode = ({ title, icon: Icon, isActive, align = 'left', subtitle, children, isVisited }) => (
  <div className={`absolute transform -translate-y-1/2 flex items-center gap-6 ${align === 'right' ? 'flex-row-reverse text-right' : 'flex-row'}`} 
       style={{ left: align === 'left' ? '5%' : 'auto', right: align === 'right' ? '5%' : 'auto', width: '45%', zIndex: 40 }}>
    ...
    <div className={`transition-all duration-500 ${isActive ? 'opacity-100 translate-x-0' : 'opacity-40'}`}>
      <h3 className="text-4xl font-bold text-white tracking-tight drop-shadow-md">{title}</h3>
...
export default function App() {
  ...
  const calculatePath = () => {
    ...
    // EXTENDED PATH TO 4350px
    const d = `M ${center} 100 L ${center} 300 Q ${center} 600 ${center - curveSize} 800 T ${center} 1400 T ${center + curveSize} 2000 T ${center} 2600 T ${center - curveSize} 3200 L ${center} 4150`;
    setPathD(d);

    const leftStart = `${center} 1400`;
    const leftEnd = `${width * 0.05 + 40} 1400`; 
    const leftBranch = `M ${leftStart} C ${center - 100} 1400, ${width * 0.15} 1400, ${leftEnd}`;

    const rightStart = `${center} 2600`;
    const rightEnd = `${width * 0.95 - 40} 2600`;
    const rightBranch = `M ${rightStart} C ${center + 100} 2600, ${width * 0.85} 2600, ${rightEnd}`;
    ...
    const animate = () => {
      ...
      if (!visitingZone) {
        ...
        if (branchPath) {
           ...
           const lerp = 0.1;
           targetPoint.x = prevPos.current.x + (mainPoint.x - prevPos.current.x) * lerp;
  ...
  return (
    <div className="fixed inset-0 bg-[#02040a] flex items-center justify-center font-sans overflow-hidden">
      ...
      {/* AMBIENT BACKGROUND */}
      <div id="ambient-background" className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-20" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M100,0 V100 H300 V300 H500 V500 H700 V700" fill="none" stroke="url(#circuit-grad)" strokeWidth="2" strokeDasharray="10,20"/>
          <path d="M0,200 H200 V400 H400 V600 H600 V800" fill="none" stroke="url(#circuit-grad)" strokeWidth="2" strokeDasharray="15,25"/>
          <path d="M600,0 V200 H400 V400 H200 V600 H0" fill="none" stroke="url(#circuit-grad)" strokeWidth="2" strokeDasharray="12,18"/>
          <circle cx="300" cy="100" r="4" fill="rgba(34,211,238,0.4)" />
          <circle cx="500" cy="300" r="4" fill="rgba(34,211,238,0.4)" />
          <circle cx="200" cy="400" r="4" fill="rgba(34,211,238,0.4)" />
          <circle cx="400" cy="200" r="4" fill="rgba(34,211,238,0.4)" />
        </svg>
        ...
        <div className="relative w-full h-[5000px]">
          ...
          <nav className="sticky top-0 w-full z-50 py-4 md:py-6 px-4 md:px-8 transition-all duration-300">
            <div className="bg-black/20 backdrop-blur-xl border border-white/5 rounded-full px-4 py-3 md:px-6 md:py-3 flex justify-between items-center shadow-lg">
              <div className="font-bold text-lg md:text-xl tracking-wider text-white flex items-center gap-2">
                ...
                <span className="hidden md:inline">PORTFOLIO<span className="text-cyan-400">.DEV</span></span>
                <span className="md:hidden">PF<span className="text-cyan-400">.</span></span>
              </div>
          ...
          <div id="work" className="absolute w-full top-[2600px] z-20 pointer-events-none">
             <MapNode title="Selected Work" subtitle="Featured Projects" icon={Code2} isActive={activeSection === 'work'} isVisited={visitState === 'arrived'} align="right">
              <div className="bg-black/60 backdrop-blur-xl p-6 rounded-3xl border border-white/10 shadow-2xl pointer-events-auto space-y-4">
                 <div className="group bg-white/5 p-5 rounded-2xl border border-white/5 hover:border-cyan-500/50 transition-all hover:bg-white/10 cursor-pointer">
                   <h4 className="text-lg font-bold text-white mb-1">E-Commerce Dashboard</h4>
                   <p className="text-sm text-slate-400">Real-time analytics platform.</p>
                 </div>
                 <div className="group bg-white/5 p-5 rounded-2xl border border-white/5 hover:border-purple-500/50 transition-all hover:bg-white/10 cursor-pointer">
                   <h4 className="text-lg font-bold text-white mb-1">Social API Service</h4>
          ...
          <div id="contact" className="absolute w-full top-[4100px] flex justify-center z-20 pointer-events-none">
            <div className="relative text-center w-full max-w-xl mx-4">
               ...
               <h2 className="text-3xl md:text-4xl font-bold mb-2 pointer-events-auto drop-shadow-lg">Let's Work Together</h2>
