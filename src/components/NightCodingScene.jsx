...
export default function NightCodingScene() {
    ...
    useEffect(() => {
        if (currentMood === MOODS.HERO) return;
    ...
    if (currentMood === MOODS.HERO) return null;
    ...
    const getThemeColors = () => {
        switch(currentMood) {
            case MOODS.AI:
            case MOODS.OS:
                return { text: 'text-cyber-violet', glow: 'drop-shadow-[0_0_8px_rgba(181,55,242,0.8)]', bgGlow: 'from-cyber-violet/30' };
            case MOODS.CYBER:
                // Hacker Green theme
                return { text: 'text-[#00ff00]', glow: 'drop-shadow-[0_0_8px_rgba(0,255,0,0.8)]', bgGlow: 'from-[#00ff00]/20' };
            case MOODS.MOBILE:
                return { text: 'text-blue-400', glow: 'drop-shadow-[0_0_8px_rgba(96,165,250,0.8)]', bgGlow: 'from-blue-500/30' };
            case MOODS.LINUX:
                // Terminal Amber theme
                return { text: 'text-[#ffb000]', glow: 'drop-shadow-[0_0_8px_rgba(255,176,0,0.8)]', bgGlow: 'from-[#ffb000]/20' };
            default:
                return { text: 'text-cyber-cyan/80', glow: 'drop-shadow-[0_0_8px_rgba(0,243,255,0.8)]', bgGlow: 'from-cyber-cyan/30' };
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden bg-cyber-dark flex flex-col justify-end opacity-80 transition-all duration-1000">
            {/* Stylized dark vignette and ambient glows to make it atmospheric */}
            <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black z-10" />
            <div className={`absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] ${theme.bgGlow} via-transparent to-transparent transition-colors duration-1000`}></div>
            ...
            <div className={`w-full h-full p-8 md:p-16 lg:p-32 font-mono text-xs md:text-sm ${theme.text} whitespace-pre overflow-hidden relative z-0 mask-image-top-fade flex flex-col justify-end transition-colors duration-1000`}>
                {visibleLines.map((line, i) => (
                    <div key={i} className={`animate-fade-in-up tracking-widest ${theme.glow} font-bold transition-all duration-1000`}>{line}</div>
                ))}
            </div>
