...
export default function HeroOverlay() {
    ...
    return (
        <section id="home" ref={heroRef} className="h-screen w-full flex items-center pointer-events-none relative z-10 overflow-hidden">
            ...
            <div ref={textRef} className="w-full max-w-7xl mx-auto px-6 md:px-12 flex flex-col lg:flex-row items-center justify-between gap-12 h-full pt-24">
                ...
                <div className="flex-1 flex flex-col justify-center pointer-events-auto text-left">
                    ...
                    <motion.div
                        ...
                        className="mb-6 inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-cyber-cyan/30 bg-cyber-cyan/5 backdrop-blur-md w-fit"
                    >
                    ...
                    >
                        <h1
                            className="text-6xl md:text-7xl lg:text-8xl font-orbitron font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white via-gray-100 to-cyber-violet glitch leading-[0.9]"
                    ...
                    <motion.h2
                        ...
                        className="text-base md:text-lg font-space text-cyber-cyan font-light tracking-[0.2em] mb-5 h-8 flex items-center"
                    >
                    ...
                    <motion.p
                        ...
                        className="max-w-[480px] text-base text-white/70 font-inter font-light tracking-wide mb-8 leading-relaxed"
                    >
                    ...
                    >
                        {[SiKotlin, SiFlutter, SiDjango, SiDocker, SiLinux].map((Icon, i) => (
                            <Icon key={i} className="w-5 h-5 hover:text-cyber-cyan hover:scale-125 transition-all duration-200 cursor-default" />
                        ))}
                    </motion.div>
                    ...
                    >
                        <a
                            ...
                            className="inline-flex items-center gap-2 bg-cyber-cyan text-black font-space font-bold uppercase tracking-widest text-sm px-7 py-3 rounded-full hover:bg-cyber-cyan/80 hover:shadow-[0_0_25px_var(--theme-primary)] transition-all"
                        >
                            View My Work
                        ...
                        <a
                            ...
                            className="inline-flex items-center gap-2 border border-white/20 text-white/80 font-space font-medium uppercase tracking-widest text-sm px-7 py-3 rounded-full hover:border-cyber-violet hover:text-cyber-violet hover:shadow-[0_0_15px_rgba(181,55,242,0.3)] transition-all"
                        >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {/* ===== RIGHT COLUMN: Holographic Terminal Panel ===== */}
                <motion.div
                    initial={{ opacity: 0, x: 60 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.9, delay: 0.4 }}
                    className="hidden lg:flex flex-col gap-4 pointer-events-auto w-[360px] shrink-0"
                >
                    {/* Stats Grid */}
                    <div className="grid grid-cols-3 gap-3">
                        {stats.map((s, i) => (
                            <motion.div
                                ...
                                className="flex flex-col items-center justify-center p-4 rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur-md text-center"
                            >
                                <span className="text-xl font-orbitron font-black text-cyber-cyan">{s.value}</span>
                    ...
                    {/* Terminal Window */}
                    <div className="rounded-2xl border border-white/10 bg-black/60 backdrop-blur-xl overflow-hidden shadow-[0_0_40px_rgba(0,247,255,0.07)]">
                        {/* Terminal header */}
                        <div className="flex items-center gap-2 px-4 py-3 border-b border-white/10 bg-white/[0.02]">
                            <div className="w-3 h-3 rounded-full bg-red-500/80" />
                            <div className="w-3 h-3 rounded-full bg-yellow-500/80" />
                            <div className="w-3 h-3 rounded-full bg-green-500/80" />
                            <span className="ml-2 text-[10px] font-space text-gray-500 tracking-widest">siluna@portfolio:~</span>
                        </div>
                        ...
                        <div className="p-4 font-mono text-xs space-y-1.5 min-h-[170px]">
                            ...
                            <motion.span
                                ...
                                className="inline-block w-2 h-4 bg-cyber-cyan align-middle"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {['Android', 'Kotlin', 'Security', 'Linux', 'Docker', 'Reverse Eng.'].map((tag) => (
                            <span
                                key={tag}
                                className="text-[10px] font-space tracking-widest uppercase px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.03] text-gray-400 hover:border-cyber-cyan/40 hover:text-cyber-cyan transition-all cursor-default"
                            >
                                {tag}
                            </span>
                        ))}
            ...
            {/* Scroll Indicator */}
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center animate-bounce opacity-50 pointer-events-none">
